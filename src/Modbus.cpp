...
word Modbus::Hreg(word offset) { return Reg(offset + 40001); }
...
word Modbus::Ireg(word offset) { return Reg(offset + 30001); }
#endif
...
void Modbus::writeMultipleRegisters(byte *frame, word startreg, word numoutputs,
                                    byte bytecount) {
  // Check value
  ...
  for (uint32_t k = 0; k < numoutputs; k++) {
...
void Modbus::writeMultipleCoils(byte *frame, word startreg, word numoutputs,
                                byte bytecount) {
  // Check value
